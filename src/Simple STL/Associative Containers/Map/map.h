...
public:
    ...
    public:
        iterator(Node* node = nullptr, const map* cont = nullptr) 
    ...
    map(const Compare& comp) : root(nullptr), sz(0), comp(comp) {}
    map(const map& other) : root(nullptr), sz(0), comp(other.comp) {
        root = copy_helper(other.root);
        sz = other.sz;
    }
    map(map&& other) noexcept : root(other.root), sz(other.sz), comp(std::move(other.comp)) {
        other.root = nullptr;
        other.sz = 0;
    }
    map(std::initializer_list<std::pair<const Key, T>> init) : root(nullptr), sz(0) {
    ...
    map& operator=(std::initializer_list<std::pair<const Key, T>> init) {
        clear();
        for (const auto& pair : init) insert(pair);
        return *this;
    }
